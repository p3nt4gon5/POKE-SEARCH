...
import { ArrowLeft, Users, Database, Heart, TrendingUp, Calendar, Activity } from 'lucide-react';
                ...
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    <div 
                      ...
                      style={{ 
                        width: `${Math.max((item.count / Math.max(...stats.userRegistrations.map(r => r.count))) * 100, 10)}px`,
                        minWidth: '10px'
                      }}
                    ></div>
              ...
              stats.mostPopularPokemon.map((pokemon, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-gray-800 capitalize">{pokemon.name}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div 
                      className="bg-green-500 h-2 rounded"
                      style={{ 
                        width: `${Math.max((pokemon.count / Math.max(...stats.mostPopularPokemon.map(p => p.count))) * 100, 10)}px`,
                        minWidth: '10px'
                      }}
                    ></div>
